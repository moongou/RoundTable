...
- 把接地校验 `validate_citation_claim` 接入实时清洗管线：`FloorManager` 新增完整
  发言历史存储 `_grounding_history`（按发言者保留原文，上限 40 条，仅实时讨论填充），
  并改造引用归属判定 `_guess_reference_owner`——优先在完整发言历史中核对“被引片段
  是否真的出现在该角色历史发言中”，确认真实出处则直接采用；若评分会命中某人但接地
  历史查无此言，则拒绝归属，避免把别人的话张冠李戴。无接地历史（单测）时保持旧行为。
- 配套新增 3 个接地校验用例（真实出处定位 / 拒绝伪造归属 / 空历史保持旧行为）。

### Bug 修复

- 修复质量审计脚本 `_mentions_participant_as_quote_owner` 漏判裸引述动词（说/讲/提/
  问/写/分享）的问题，使 `小爱说“……”` 这类显式错引能被审计正确识别。

### 验证

- `backend/.venv/bin/python -m pytest backend/tests -q`：323 passed。
