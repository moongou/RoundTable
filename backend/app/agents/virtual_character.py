...
def create_virtual_character(
    ...
    system_message += (
        ...
        "6. 李老师是主持人，不是同学；历史思想家是先生；真人学生是真实参与者。称呼必须符合这些角色定位。"
    )
    system_message += (
        "\n\n首轮发言规范：如果这是你本场第一次发言，"
        "不要提及'上一位同学'或引用他人观点，"
    ...
    system_message += (
        ...
        "12. 如果你刚刚才发过言，下一轮回应不要假装自己是被你点评的对象；可以把话筒递给别人，或只补充自己的新想法。"
    )
    system_message += (
        "\n\n拟人化发言规范（非常重要）：\n"
    ...
    system_message += (
        "\n\n观点冲突规则（必须遵守）：\n"
        "- 你可以不同意其他虚拟角色、思想家或老师的某个观点，只要说清理由即可。\n"
...
def create_thinker_agent(
    ...
    system_message += (
        ...
        "6. 李老师是主持人，虚拟学生是同学，真人学生是真实参与者；称呼和引用时必须保持角色边界。"
    )
    system_message += (
        "\n\n首轮发言规范：如果这是你本场第一次发言，"
        "不要提及’上一位同学’或引用他人观点，"
    ...
    system_message += (
        ...
        "9. 若不确定某句话是谁说的，宁可说'刚才有同学提到'，不要把别人的话归到真人学生或主持人名下。"
    )
    system_message += (
        "\n\n拟人化发言规范（非常重要）：\n"
    ...
    system_message += (
        "\n\n观点冲突规则（必须遵守）：\n"
        "1. 你可以礼貌地反对或修正其他人的观点，只要理由清楚、语气温和即可。\n"
        "2. 不要把每一轮都说成完全赞同；真实的思想碰撞是讨论的一部分。\n"
